...
/// Does not free memory allocated by the writer, use 'scar_writer_free' for that.
int scar_writer_finish(struct scar_writer *sw);

/// Free a scar_writer.
/// Does not free the 'scar_compression' or 'scar_io_writer' that was passed in to the
/// scar_writer_create function; those must be freed separately.
