...
/// Will create exactly one 512-byte USTAR header block if the metadata is fully
/// representable using the USTAR format,
/// or one 'x'-type PAX extended header followed by a 512-byte USTAR header block if
/// the metadata isn't fully representable using the USTAR format.
